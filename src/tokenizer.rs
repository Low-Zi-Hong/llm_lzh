...
impl Tokenizer {
    pub fn new(vocab_path: &str) -> Self {
        ...
        let vocab_value : Value = serde_json::from_str(&vocab_raw_str).expect("cannot convert to str");
        ...
        if let Some(vocab) = vocab_value["added_tokens"].as_array(){
            ...
            vocab.iter().for_each(|x| {
                let string = x["content"].as_str().expect("cannot get content").to_string();
        ...
        let token_map= Tokenizer { special_map: special ,encode_map: encoder, decode_map: decoder,byte_decoder: bytedecoder, buffer: vec![] };
